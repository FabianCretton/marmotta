...
function ExtDataSources(serverUrl) {
	...
	if (serverUrl == undefined)
		throw "url must be defined"; //test if url is defined
	if (serverUrl.lastIndexOf("/") == serverUrl.length - 1)
		serverUrl = serverUrl.substring(0, serverUrl.length - 1); //clean url
	...
	var options = {
		...
		filtersList : {
			path : serverUrl + "/EDS/filters/list"
		},
		validatorsList : {
			path : serverUrl + "/EDS/validators/list"
		}
	}
	...
	/**
	 * Get the list of data filters files available on the server
	 * @param onsuccess Function is executed on success with a JSON object which is a simple string list file names (OPTIONAL)
	 * @param onfailure Function is executed on failure. It takes a JQuery jqXhr object.(OPTIONAL)
	 */
	this.getDataFiltersList = function (onsuccess, onfailure) {
		$.getJSON(options.filtersList.path, function (data) {
			if (onsuccess)
				onsuccess(data);
			else
				console.debug("getDataFiltersList successful");
		}).error(function (jqXhr, textStatus, error) {
			if (onfailure)
				onfailure(jqXhr);
			else
				console.debug("getDataFiltersList failed (" + jqXhr.statusText + ": " + jqXhr.responseText + ")");
		});
	}
	
	/**
	 * Get the list of data validators files available on the server
	 * @param onsuccess Function is executed on success with a JSON object which is a simple string list file names (OPTIONAL)
	 * @param onfailure Function is executed on failure. It takes a JQuery jqXhr object.(OPTIONAL)
	 */
	this.getDataValidatorsList = function (onsuccess, onfailure) {
		$.getJSON(options.validatorsList.path, function (data) {
			if (onsuccess)
				onsuccess(data);
			else
				console.debug("getDataValidatorsList successful");
		}).error(function (jqXhr, textStatus, error) {
			if (onfailure)
				onfailure(jqXhr);
			else
				console.debug("getDataValidatorsList failed (" + jqXhr.statusText + ": " + jqXhr.responseText + ")");
		});
	}
